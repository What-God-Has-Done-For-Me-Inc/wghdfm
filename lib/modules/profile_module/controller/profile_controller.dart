...
class ProfileController extends GetxController {
  ...
  Future<void> onPressBlock(BuildContext context) async {
    blocUserApi(context: context, onPressUnBlock: onPressUnBlock, profileId: userId, userId: profileId);
  }

  RxList<PostModelFeed>? profileFeeds = <PostModelFeed>[].obs;

  Future getProfileFeed({bool isFirstTimeLoading = false, var page = 1, required var profileId}) async {
    isProfileLoading.value = true;
    print(">> PROFILE DATA API CALLING");
    await APIService().callAPI(
      params: {},
      serviceUrl: isFirstTimeLoading ? "${EndPoints.baseUrl}wire/index_profile/$profileId" : "${EndPoints.baseUrl}wire/more_profile/$page/$profileId",
  ...
  Future getProfileData({required String profileID, Function? callBack, bool? showLoader}) async {
    APIService().callAPI(
        params: {},
        formDatas: dio.FormData.fromMap({"user_id": profileID, "profile_id": profileID}),
        ...
        success: (dio.Response response) {
          ...
          userName.value = ("${profileDetails?.value.firstname ?? ""} ${profileDetails?.value.lastname ?? ""}").trim();
  ...
  Future updateProfileImage({required File image, bool? isProfile, required String userIdCurrent}) async {
    await APIService().callAPI(
      params: {},
      ...
      serviceUrl: EndPoints.baseUrl + (isProfile == true ? EndPoints.updateProfileImage : EndPoints.updateCoverImage),
      method: APIService.postMethod,
      success: (dio.Response response) async {
  ...
  Future getSomeonesProfileData({required String profileID}) async {
    ...
    await APIService().callAPI(
        params: {},
        formDatas: dio.FormData.fromMap({"user_id": profileID, "profile_id": userID}),
        serviceUrl: EndPoints.baseUrl + EndPoints.userDetails,
        method: APIService.postMethod,
        success: (dio.Response response) {
  ...
  Future blockUser({required userID, required Function callBack}) async {
    ...
    await APIService().callAPI(
        params: {},
        formDatas: dio.FormData.fromMap({"user_id": userID, "profile_id": myProfileId}),
  ...
  Future addMessage({required userID, required String message, required Function callBack}) async {
    dynamic userDetails = await SessionManagement.getUserDetails();
    String myProfileId = userDetails.id;
    await APIService().callAPI(
        params: {},
        formDatas: dio.FormData.fromMap({'user_id': myProfileId, 'receipt': userID, 'message': message}),
  ...
  Future changePassword({
    ...
    await APIService().callAPI(
        params: {},
        formDatas: dio.FormData.fromMap({"user_id": userID, "old_password": oldPassword, "password": newPassword, "con_password": newPassword}),
        ...
        success: (dio.Response response) {
          ...
          if (responseBody["type"] == "success") {
            ...
            snack(title: "Opps", msg: '${responseBody["text"]}', iconColor: Colors.red, icon: Icons.close);
          }
        ...
        error: (dio.Response response) {
          ...
          Get.back();
          snack(title: "Failed", msg: '${response.data["text"]}', iconColor: Colors.red, icon: Icons.close);
        },
        showProcess: true);
  }
  ...
  Future deleteAccount({
    ...
    await APIService().callAPI(
        params: {},
        formDatas: dio.FormData.fromMap({"user_id": userID, "password": password}),
        ...
        success: (dio.Response response) {
          ...
          if (responseBody["type"] == "success") {
            ...
            snack(title: "Opps", msg: '${responseBody["message"]}', iconColor: Colors.red, icon: Icons.close);
          }
        },
        error: (dio.Response response) {
          dynamic responseBody = jsonDecode(response.data);
