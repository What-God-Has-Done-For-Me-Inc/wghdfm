...
import 'package:wghdfm_java/modules/agora_module/controller/agora_controller.dart';
...
class SessionController extends ValueNotifier<AgoraSettings> {
  ...
  String getUserName({required int uid, required List activeUsers}) {
    print(uid);
    String name = "";
    activeUsers.forEach((userData) {
      if (userData["uid"] == uid.toString()) {
        var user_name = userData["user_name"].toString();
        name = user_name.split(" ").first.toString();
      }
    });
    return name;
  }
